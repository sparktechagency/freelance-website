...
import { doctorAvailableService } from './doctorBooking.service';
...
const getSingleDoctorAvailableSlots = catchAsync(async (req, res) => {
  const doctorId = req.params.doctorId;
  const date:any = req.query.date;
  const result = await doctorAvailableService.getSingleDoctorAvailableSlotsQuery(
    doctorId,date
  );
  ...
  sendResponse(res, {
    ...
    statusCode: httpStatus.OK,
    data: result,
    message: 'Single DoctorAvailable Slots are requered successful!!',
  });
});
